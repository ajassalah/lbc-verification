...
export default function Edit({ auth, learner, genderOptions, proofTypeOptions, countryOptions, nationalityOptions = [], nationalityDialCodes = {} }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        name_with_initials: learner.name_with_initials || '',
        date_of_birth: learner.date_of_birth || '',
        profile_picture: '',
        gender: learner.gender || '',
        email: learner.email || '',
        learner_id: learner.learner_id || '',
        proof_type: learner.proof_type || '',
        proof_id: learner.proof_id || '',
        id_proof_document: '',
        cv_document: '',
        phone_no: learner.phone_no || '',
        address_line_1: learner.address_line_1 || '',
        address_line_2: learner.address_line_2 || '',
        city: learner.city || '',
        state: learner.state || '',
    ...
    const handleProfilePictureChange = (e) => {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                ...
                console.log("New preview URL set:", e.target.result);
            };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('learners.update', learner.id));
    };
        ...
        <AuthenticatedLayout
            ...
            header={
                <div className="flex justify-between items-center">
                    <h2 className="font-semibold text-xl text-gray-800 leading-tight">Edit Learner</h2>
                </div>
            }
        >
                    ...
                    >
                        <div className="p-6 bg-white border-b border-gray-200">
                            <form onSubmit={handleSubmit} encType="multipart/form-data">
                                ...
                                >
                                    <h3 className="text-lg font-medium text-gray-900 border-b pb-2 mb-4">Profile Picture</h3>
                                    <div className="flex flex-col items-center">                                        <div className="w-40 h-40 rounded-full overflow-hidden bg-gray-100 mb-4 flex items-center justify-center">                                            {previewUrl ? (
                                        <img
                                            src={previewUrl}
                                            alt="Profile preview"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    >
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            {/* Learner ID */}
                                            <div>
                                                <InputLabel htmlFor="learner_id" value="Learner Number /Reference *" />
                                                <TextInput
                                                    id="learner_id"
                                                    type="text"
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                            {/* Name with Initials */}
                                            <div>
                                                <InputLabel htmlFor="name_with_initials" value="Name with Initials *" />
                                                <TextInput
                                                    ...
                                                    required
                                                />
                                                <InputError message={errors.name_with_initials} className="mt-2" />
                                            </div>
                                    ...
                                    >
                                        <h3 className="text-lg font-medium text-gray-900 border-b pb-2 mb-4">Identification</h3>
                                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                            ...
                                            {/* Proof Type */}
                                            <div>
                                                <InputLabel htmlFor="proof_type" value="ID Type *" />
                                                <SelectInput
                                                    id="proof_type"
                                                    className="mt-1 block w-full"
                                                ...
                                                >
                                                    ...
                                                    {proofTypeOptions.map((type, index) => (
                                                        <option key={index} value={type}>{type}</option>
                                                    ))}
                                                </SelectInput>
                                                <InputError message={errors.proof_type} className="mt-2" />
                                            </div>

                                            {/* Proof ID */}
                                            <div>
                                                <InputLabel htmlFor="proof_id" value={proofIdLabel} />
                                                <TextInput
                                                    id="proof_id"
                                                    type="text"
                                                    className="mt-1 block w-full"
                                                    value={data.proof_id}
                                                    onChange={(e) => handleChange('proof_id', e.target.value)}
                                                    required
                                                />
                                                <InputError message={errors.proof_id} className="mt-2" />
                                            </div>                                        </div>
                                    </motion.div>
                                    ...
                                    >                                        <h3 className="text-lg font-medium text-gray-900 border-b pb-2 mb-4">
                                        ...
                                        <div className="mb-4">
                                            <p className="text-sm text-gray-700 p-2 bg-blue-50 border border-blue-100 rounded-md">
                                                <span className="font-semibold">Note:</span> Your existing documents will be preserved unless you upload new ones. Only upload new documents if you want to replace the current ones.
                                            </p>
                                        </div>
                                            ...
                                            <div>
                                                ...
                                                {learner.id_proof_document && (
                                                    <div className="mt-2">
                                                        ...
                                                        >
                                                            View current ID proof document
                                                        </a>
                                                    </div>
                                                )}                                                <p className="mt-1 text-sm text-gray-500">Upload PDF, PNG, or JPEG maximum 20MB (only if you want to change it)</p>
                                                <InputError
                                    ...
                                    <motion.div
                                        ...
                                        className="md:col-span-2"
                                    >
                                        <h3 className="text-lg font-medium text-gray-900 border-b pb-2 mb-4">Address Information</h3>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {/* Address Line 1 */}
                                            <div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                                            ...
                                            {/* Postal Code */}
                                            <div>
                                                <InputLabel htmlFor="postal_code" value="Postal/ZIP Code" />
                                                <TextInput
                                                    id="postal_code"
                                                    type="text"
                                                    className="mt-1 block w-full"
                                ...
                                >
                                    ...
                                    <motion.div variants={buttonVariants} whileHover="hover">
                                        <PrimaryButton type="submit" className="ml-4" disabled={processing}>
                                            {processing ? 'Updating...' : 'Update Learner'}
                                        </PrimaryButton>
                                    </motion.div>
                                </motion.div>
                            </form>
